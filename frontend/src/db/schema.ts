...
export const routineSets = sqliteTable('routine_sets', {
  ...
  // Adaptive Training additive columns
...
// ---------------------------------------------------------------------------
// Adaptive Training additive tables (PR1/PR2)
// ---------------------------------------------------------------------------

export const readinessCheckIns = sqliteTable('readiness_check_ins', {
  id: text('id').primaryKey(),
  recordedAt: text('recorded_at').notNull(),
  sleepQuality: integer('sleep_quality'),
  generalFatigue: integer('general_fatigue'),
  motivation: integer('motivation'),
  stress: integer('stress'),
  sorenessJson: text('soreness_json'),
  painRegionsJson: text('pain_regions_json').notNull().default('[]'),
  unusualEvent: text('unusual_event'),
});

export const recoverySnapshots = sqliteTable('recovery_snapshots', {
  id: text('id').primaryKey(),
  recordedAt: text('recorded_at').notNull(),
...
export const externalTrainingSessions = sqliteTable('external_training_sessions', {
  id: text('id').primaryKey(),
  ...
  affectedRegionsJson: text('affected_regions_json'),
  source: text('source').notNull(),
});

export const trainingRecommendations = sqliteTable('training_recommendations', {
  id: text('id').primaryKey(),
  generatedAt: text('generated_at').notNull(),
  recommendationType: text('recommendation_type').notNull(),
  exerciseId: text('exercise_id'),
  prescribedLoad: real('prescribed_load'),
  prescribedRepMin: integer('prescribed_rep_min'),
  prescribedRepMax: integer('prescribed_rep_max'),
  prescribedSets: integer('prescribed_sets'),
  targetRir: real('target_rir'),
  action: text('action').notNull(),
  rationaleCodesJson: text('rationale_codes_json').notNull().default('[]'),
  humanReadableReason: text('human_readable_reason'),
  inputsUsedJson: text('inputs_used_json').notNull().default('[]'),
